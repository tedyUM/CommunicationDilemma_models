...
k_f = 1;    % Self negative feedback strength
...
for i = 1:(length(t) - 1)
    ...
    if i > tau
        f(i+1,1) = f(i,1) + dt * (-f(i,2) + k_cf*(c(i-tau,1) - f(i,1)) - k_f*f(i-tau,1));
        f(i+1,2) = f(i,2) + dt * (f(i,1) + k_cf*(c(i-tau,2) - f(i,2)) - k_f*f(i-tau,2));
    else
