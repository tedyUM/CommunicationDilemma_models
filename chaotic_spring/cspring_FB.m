...
for i = 1:(length(t) - 1)
    ...
    if i> tau
    % Up1ate c
        c(i+1,1) = c(i,1) + dt * (-c(i,2) + k_sc*(s(i,1) - c(i,1)) +k_cf*(f(i-tau,1) - c(i,1) ) );
        c(i+1,2) = c(i,2) + dt * (c(i,1) + k_sc*(s(i,2) - c(i,2)) +k_cf*(f(i-tau,2) - c(i,2) ) );
    else
        c(i+1,1) = c(i,1) + dt * (-c(i,2) + k_sc*(s(i,1) - c(i,1)) +k_cf*(f(i,1) - c(i,1) ) );
        c(i+1,2) = c(i,2) + dt * (c(i,1) + k_sc*(s(i,2) - c(i,2)) +k_cf*(f(i,2) - c(i,2) ) );
